...
void *producer(void *data) {
	...
	while (1) {
		...
		if (noOfFreeSpacesQ() == 0) {
			...
			printf("<< Any time, Press ctrl+z to stop the program and ctrl+c to get manager menu >>\n");
		...
		printf("\n-----Going to produce %d item(s)-----\n", numItems);
		enQ(numItems);
		printf("-----%d item(s) are produced-----\n",numItems);
...
void *consumer(void *data) {
	...
	while (1) {
		...
		printf("\n-----Going to consume %d item(s)-----\n",numItems);
		deQ(numItems);
		printf("-----%d item(s) are consumed-----\n", numItems);
